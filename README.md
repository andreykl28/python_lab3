...
Алгоритмический мини‑пакет (структуры данных и сортировки): функции факториала и Фибоначчи, набор сортировок, стек/очередь, генераторы тестовых данных и CLI для бенчмарков.

Цель — реализовать требуемые алгоритмы и обвязку для варианта Medium с поддержкой `key`/`cmp`, валидацией входных данных и тестами.
...
```
Текущее покрытие ~96% (pytest‑тесты в каталоге `tests`).
